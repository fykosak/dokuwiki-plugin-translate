...
class action_plugin_translate extends DokuWiki_Action_Plugin {
    ...
    public function register($contr) {
        $contr->register_hook('ACTION_ACT_PREPROCESS', 'BEFORE', $this, 'handleInitLangByDomain');
        $contr->register_hook('ACTION_ACT_PREPROCESS', 'BEFORE', $this, 'handleInitLangByPageInfo');
    ...
    public function handleInitLangByDomain() {
        global $conf;
        static $map = array(
            'org' => 'en',
            'cz' => 'cs',
        ...
        //$event->data = isset($map[$matches[1]]) ? $map[$matches[1]] : 'cs';
        $l = isset($map[$matches[1]]) ? $map[$matches[1]] : 'cs';
        $conf['lang'] = $l;
    }

    public function handleInitLangByPageInfo() {
        global $INFO, $conf;
        $conf['lang'] = $this->helper->getPageLanguage(null, null);
        $langs = array();
        if ($INFO['exists']) {
            if ($this->helper->isTranslatable()) {
                $orig = $this->helper->getOriginal();
                $origlang = $this->helper->getPageLanguage($orig);
                $langs = array_values($this->helper->getTranslations($orig));
                // $currentlang = $this->helper->getPageLanguage($ID);
        ...
        $conf['available_lang'] = array();
        foreach ($langs as $lang) {
            $text = $this->helper->getLanguageName($lang);
            $conf['available_lang'][] = [
                'content' => $this->helper->translationLink($lang, $text, false),
                'code' => $lang
            ];
        }
    }
    ...
    public function handleDokuwikiStarted() {

        global $INFO;
        $info = &$INFO;

        // Is the user blocked from editing the page?
        if (empty($_SERVER['REMOTE_USER']) || empty($INFO['meta']['relation']['istranslationof']) || $info['perm'] < AUTH_READ || $info['perm'] >= AUTH_EDIT) return;
     ...
     */
    public function handleActPreprocess(Doku_Event&$event) {
        $act = $event->data;
        if (is_array($act)) {
        ...
        switch ($act) {
            case 'createpage':
                $this->handleActPreprocessCreatepage();
                break;
            case 'translate':
                $this->handleActPreprocessTranslate($event);
                break;
            default:
     ...
     */
    public function handleTplActUnknown(Doku_Event&$event) {
     ...
     */
    public function handleHtmlEditformOutput($event) {
        global $INFO;
        ...
        $origelem = '<textarea id="translate__sourcetext" ' . //buildAttributes($attrs,true).
            'class="edit" readonly="readonly" cols="80" rows="10"' . 'style="width:100%;"' .//  height:600px; overflow:auto
            '>' . NL . hsc($origtext) . '</textarea>';
    ...
    public function handleActPreprocessCreatepage() {
        global $INFO;
        /**
         * @var $my helper_plugin_translate
         */
        ...
        if (!$auth_ok && $auth >= AUTH_READ) {
            ...
            $auth_ok = !empty($grp) && in_array($grp, $INFO['userinfo']['grps']);
        }
        if (!$auth_ok) {
    ...
    public function handleActPreprocessTranslate(Doku_Event $event) {
        global $ID, $INFO;
        /**
         * @var $my helper_plugin_translate
         */
