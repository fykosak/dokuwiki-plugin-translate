...
class helper_plugin_translate extends DokuWiki_Plugin {
    ...
    public function translationLink($language,$text = '',$hsc = true) {
    ...
    public function translationLinks() {
        ...
        else return $this->translationLinksAll();
    }
            ...
            if(empty($langs)){
                ...
                if($handle = opendir(DOKU_INC.'inc/lang')){
                    while (false !== ($file = readdir($handle))) {
                        if($file[0] == '.') continue;
                        if(is_dir(DOKU_INC.'inc/lang/'.$file)) array_push($langs,$file);
                    }
                    closedir($handle);
                }
                sort($langs);
    ...
    public function printActionTranslatePage() {
        ...
        $form->addElement(form_makeTextField('',$this->getLanguageName($source_lang),$this->getLang('original_language'),'','',array('readonly' => 'readonly')));
        ...
        $form->addElement(form_makeTextField('',p_get_first_heading($ID),$this->getLang('original_title'),'','',array('readonly' => 'readonly')));
